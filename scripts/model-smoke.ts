 ...
 * ## 但它有一条会红，而那条红要读准
 *
 * 「usage 记到了数」的前提是**这一轮真的调通了**。如果模型把自己的解码搞崩
 * （实测 llama3.2:3b 会退化成重复输出、被 Ollama 的重复保护掐掉），这一条会红 ——
 * 它在说的是「**这一轮没跑完**」，不是在说「模型选得不准」。
 * 两者的处理方式完全不同：前者这个模型在你机器上不能用，后者换个模型/改 prompt。
 * 所以红的时候一定连着看上面那行降级原因，别只看红。
 */
